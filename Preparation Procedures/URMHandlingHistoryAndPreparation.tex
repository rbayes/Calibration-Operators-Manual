...
\item With an M5 hex and crescent wrench, remove the bolts holding the URM cover flange to the base. Store the nuts, bolts and washers so that they can be easily found and used. 
...
\item Inspect the outside of the umbilical for damage.
...
\item Disassemble the front and rear pulley blocks, the rope encoder
  ...
  pulleys from the umbilical motor assembly. Be careful removing the
  bearings from the guide pulleys in particular as there is some
  potential for grease on the surfaces. Note that no grease was
  observed on the outside surfaces of the bearings from the
  disassembly of URM5, but some caution is warrented.
...
\item Reassemble the front and rear pulley blocks, the rope pulleys,
  and the pulleys on the motor assembly using the parts from
  URM5. URM5 is physically identical to URM4, and the pulleys and
  their support systems have already been cleaned ultrasonically, so
  the blocks, and rope pulleys can just be replaced. Similarly the
  pulleys on the URM4 motor assembly should be replaced with the clean
  ones from URM5.
...
there is an additional length of at least 2 m on the running end of
the umbilical to accommodate the consolidation of the fibre bundle and
the installation of the source; more is likely better as having a
length of 3~m (suggested) at the running end will allow the end of the
umbilical to be comfortably manipulated for the sealing of the fibre
bundle ends while the remaining 4~m at the standing end might not
reach the dye laser without the use of an additional patch cable. The
position of the umbilical ends is dictated by the position of the
feed-through plate. The requested 4~m of standing end slack will be
sufficient to allow the fibre bundle consolidation to be done on a
comfortable surface adjacent to the URM cart.
...
\item Identify the location of the feed-through on the umbilical. This
  should be 4 meters from the end of the umbilical to provide space to
...
The required parts for this procedure are
\begin{itemize}
\item Umbilical installed on URM with fibre bundle
\item Source pivot (assembled)
\item Fibre lock plate (assembled)
\item Source Connector upper cylinder
\item Source Connector bottom cylinder
\item Source Connector nut
\item Source Connector top plate
\item source connector end groove FFKM o-rings
\item source connector side wall FFKM o-rings
% \item TEV o-ring for AmBe stem plate
\item 3x A-122 Teflon o-rings
\item 3x pressure plates
\item 16x M3-5mm screws
% \item 12x M3-14mm screws
\item 6x \#4-40 3/4'' screws
\end{itemize}
...
%  method of tying the knot described above (see Fig.\ref{fig:altbowline}) and should be tied so that the knot cannot go above the edge of the rope groove on the barrel.
% \end{answerlist}
...
\item Thread the umbilical through the three pressure plates with a teflon o-ring following each one.
\item Thread the umbilical through the top plate of the source connector.
\item Ensuring a surplus of umbilical length is available below the source connector top plate, leave 1 - 3m of umbilical for the completion of the fibre bundle.
\item Wrap clean room plastic sheets around the source tube components. Heat seal or use zip ties to seal the plastic around the source connector components.
\item Wrap the umbilical in the clean room plastic sheeting. Cover the entire umbilical from the URM to the tygothane cladding. At this point the source connector components have a double layer of protection.
\item Expose $\sim$5cm of Tygothane cladding below the end of the umbilical followed by 20 cm of exposed fibre.
\item Feed heat shrink wrap onto the fibres serving as additional fibre protection at the top of the fibre bundle.
\item Feed the spring cap followed by the fibre spring through the exposed fibres. If desired, wrap these pieces in plastic to protect from debris during the fibre cutting and polishing. \textit{Note: all these components will be cleaned with UPW afterwards and are sealed within the source connector.}
\item Prepare the station for completing the fibre bundle. Have the fibre cable shroud, epoxy, and a 1 mL plastic syringe ready.
\item Mix the epoxy as directed and draw $\sim$1 mL into the plastic syringe. Slide the fibre cable shroud onto the fibres as you inject the epoxy continually until the bundle is filled.
\item Ensure all fibres reach the end of the fibre bundle. Leave the fibre bundle for curing. \textit{Note: this may take several days but can be expedited by setting up a heat gun to provide a warmer environment.}
\item Once cured, set up the station for cutting the fibre bundle. This involves a dremel with a diamond dremel bit, plastic bundle rig, and vacuum. Secure the dremel with a c-clamp and install the diamond bit.
...
\item Prepare the polishing station. It is helpful to have a retort stand and clamp hold the umbilical upright with the fibre bundle directed downwards. This allows for the sandpaper to be placed underneath and the fibre bundle goes into the sanding puck while minimizing the load on the bundle.
\item Using UPW to wet the carbide sandpaper progress from 400 to 4000 grit sandpaper in progressive increments. At 4000 grit, use the 1 micron polishing compound. After polishing a quick test with a cap lamp at one end can verify quality.
\item The fibre bundle is now complete. Wipe down the exposed fibres (gently) with UPW and use a KimWipe to dry. Additionally, wipe the exterior of the protective plastic wrapping to contain debris that may have attached to the exterior.
\item Remove the plastic wrapping of the spring and spring cap components. Bring the spring and spring cap to the fibre bundle.
\item Secure the fibre connector top plate to the source upper source connector body with 8 M3 screws.
\item Connect the wires to the further electical feed throughs on the source connector plate.
\item Couple the end of the fibre via two M3 screws on spring cap onto the fibre connector top plate. There should be enough length to allow for slack on the fibres. The additional slack accommodates potential umbilical stretching, which has been observed in previous deployments.
\item Remove the protective plastic wrapping and slide it down the umbilical to fasten the source connector top plate to the top of upper source connector body using 8 M3 screws. These screws must include holes for retaining wire. The retaining wire should be installed at this time.
\item Tighten the pressure plates against the top of the source
  connector top plate.  Using six \#4-40 3/4 inch length screws until
  there is an equal gap less than 1 mm between all of the plates. The
  mounting screws must have a hole for a retaining wire, and that wire
  should be installed after
\item Secure the source pivot pulley support plates to the UFO assembly using four M3 screws. Retaining wire should be strung between the screws, which should also be equipped with holes to accommodate the wire. 
...
\begin{figure}
  \includegraphics[width=\textwidth]{AltPrepBowline}
  \caption{The suggested alternate method of tying a bowline for the pivot assembly and the rope spool. Starting with a loop in the rope, fold a bight into the loop on the standing end. Then pass the running end through the bight to create the final loop at the end of the rope. The bight can be tightened somewhat to remove slack. The running end can be tightened against the bight so the terminal loop is the desired diameter. Once the terminal loop is set properly, the standing end can be pulled tight. This draws the running end through the inital loop to make the classic bowline shown in Fig \ref{fig:bowline}}
  \label{fig:altbowline}
\end{figure}
...
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
  \includegraphics[width=\textwidth]{ReplacementUFOCylinder1}
  \caption{The replacement source connector top plate}
  \label{fig:newTopPlate}
\end{subfigure}
\begin{subfigure}{0.45\textwidth}
  \includegraphics[width=\textwidth]{AssembledSourceConnector}
  \caption{Source Connector pieces assembled with the fibre cable lock plates}
  \label{fig:assembledConnector}
\end{subfigure}
\caption{The original plan to terminate the umbilical includes the source pulley assembly, a umbilical flasher object and a source connector as shown. The replacement top plate mechanically replaces the UFO top plate, acrylic housing and steel cylinder.}
\end{figure}
...
\subsection{Bellows installation}\label{sss:BellowsInstall}
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.4\textwidth]{URMLiftingTableSafetyBars}
  \end{center}
  \caption{Location of the URM safety stop bars on the URM lifting cart}
  \label{fig:URMSafeStop}
\end{figure}
...
\item With the URM at operating height, install the turnbuckles installed on the hangers to fix the URM rails into position. Set the safety stop bars in place at the base of the table as shown in Fig. \ref{fig:URMSafeStop}.
